...
# initialise everything in pygame package
pygame.init()
fps = 60
...
def draw_score():
    ...
    if mode == 2:
        mode_text = font.render(f'Time Elapsed: {time_remaining}', True, 'black')
...
def check_shots(targets, coords):
    global points
    mouse_pos = pygame.mouse.get_pos()
...
def draw_menu():
    global game_over, ammo, pause, mode, level, menu, time_passed, total_shots, points, time_remaining
    global best_freeplay, best_ammo, best_timed, write_values, clicked, new_coords
    game_over = False
    pause = False
    ...
    screen.blit(font.render(f'{best_freeplay}', True, 'black'), (330, 580))
...
def draw_game_over():
    global level, pause, game_over, menu, points, total_shots, time_passed, clicked, time_passed, time_remaining
    if mode == 0:
...
def draw_pause():
    global level, pause, menu, points, total_shots, time_passed, clicked, time_passed, time_remaining, new_coords
...
while run:
    ...
    if level == 1:
        ...
        if shot:
            one_coords = check_shots(target_boxes, one_coords)
            shot = False
        ...
        if shot:
            two_coords = check_shots(target_boxes, two_coords)
            shot = False
        ...
        if shot:
            three_coords = check_shots(target_boxes, three_coords)
            shot = False
